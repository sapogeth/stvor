    ...
    <main className="min-h-screen flex flex-col bg-black text-white">
      ...
      <div className="flex-1 p-8">
        <div className="max-w-4xl mx-auto">
          {error && (
            <div className="mb-6 p-4 bg-red-900/30 border border-red-500 text-red-300 rounded-lg">{error}</div>
          )}

          <div className="bg-gray-900 border border-gray-800 rounded-xl shadow-2xl overflow-hidden mb-8">
            <div className="flex border-b border-gray-800">
              <button onClick={() => setActiveTab('plan')} className={'flex-1 px-6 py-4 font-semibold ' + (activeTab === 'plan' ? 'text-green-500 border-b-2 border-green-500' : 'text-gray-400 hover:text-green-500')}>Plan</button>
              <button onClick={() => setActiveTab('usage')} className={'flex-1 px-6 py-4 font-semibold ' + (activeTab === 'usage' ? 'text-green-500 border-b-2 border-green-500' : 'text-gray-400 hover:text-green-500')}>Usage</button>
              <button onClick={() => setActiveTab('billing')} className={'flex-1 px-6 py-4 font-semibold ' + (activeTab === 'billing' ? 'text-green-500 border-b-2 border-green-500' : 'text-gray-400 hover:text-green-500')}>Billing</button>
              <button onClick={() => setActiveTab('api')} className={'flex-1 px-6 py-4 font-semibold ' + (activeTab === 'api' ? 'text-green-500 border-b-2 border-green-500' : 'text-gray-400 hover:text-green-500')}>API Keys</button>
            </div>
                ...
                {activeTab === 'plan' && data && (
                  <div className="p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <button className="w-full bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-green-500 text-white font-semibold py-3 rounded-lg transition text-green-500 hover:text-green-400">Upgrade Plan</button>
                    </div>
                  </div>
                )}
                    ...
                    {data?.billing?.length ? (
                      <ul className="space-y-2">
                        {data.billing.map((b: any) => (
                          <li key={b.id} className="flex justify-between p-3 border border-gray-700 rounded bg-gray-800">
                            ...
                            <span className={"ml-3 text-xs px-3 py-1 rounded-full " + (b.status === 'paid' ? 'bg-green-900/30 text-green-400' : b.status === 'pending' ? 'bg-yellow-900/30 text-yellow-400' : 'bg-red-900/30 text-red-400')}>{b.status}</span>
                          </li>
                        ))}
                      </ul>
                    ) : (
                ...
                {activeTab === 'api' && (
                  <div className="p-8">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <button onClick={genKey} disabled={generating} className="bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-green-500 disabled:bg-gray-700 text-green-500 hover:text-green-400 font-semibold py-2 px-4 rounded-lg transition">
                    ...
                    {apiKeys.length === 0 ? (
                      ...
                      <div className="space-y-4">
                        {apiKeys.map((k) => (
                          <div key={k.id} className="border border-gray-700 rounded-lg p-4 bg-gray-800">
                            <div className="flex justify-between items-start mb-3">
                              ...
                              <span className="px-3 py-1 rounded-full text-xs font-semibold bg-green-900/30 text-green-400">Active</span>
                            </div>
                            <div className="bg-gray-950 p-3 rounded font-mono text-sm break-all mb-4 text-gray-300">{k.key}</div>
                          </div>
                        ))}
                      </div>
                    )}
